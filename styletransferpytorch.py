...
# TODO: Think about refactoring this piece to work with any model in general
def rebuild_vgg19(vgg19, content_image, style_image):
    ...
    # Create a new model that will be modified version of vgg19
    # starts with normalization layer to ensure all images that are
    # inserted are normalized like the ones vgg was trained on
    mean = [0.485, 0.456, 0.406]
    std = [0.229, 0.224, 0.225]

    norm_layer = NormalizeLayer(mean, std).to(device)
    ...
    for layer in vgg19.children():
        ...
        # TODO: This is so far the only way I've found to do this, maybe there's better?
...
# 6. Write training function
def style_transfer(vgg19, content_image, style_image, input_image, num_steps=300,
                   style_weight=100000, content_weight=1):
    '''Runs the style transfer on input image'''
    # Get the rebuilded model and style and content layers
    model, content_layers, style_layers = rebuild_vgg19(vgg19, content_image, style_image)
...
if __name__ == '__main__':
    ...
    vgg19_cnn = models.vgg19(pretrained=True).features.to(device).eval()
    pprint.pprint(vgg19_cnn)
    ...
    output = style_transfer(vgg19_cnn, content_tensor, style_tensor, input_tensor)
